...
void vulnerable2(void) {
    ...
    while (1) {
        ...
        //Note this payload may change every build due to because the linker may place blink() and the stack frame at different addresses. Re-check with GDB/nm., so adjust the last two words accordingly based on the observed stack layout in GDB.
